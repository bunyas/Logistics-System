...
{  
    public class OrderRutfModels
    {
        ...
        public Order_RUTF_Details GetRecordByKey(string order_no, int? product_code)
        {
            try
            {
                return context.Order_RUTF_Details.FirstOrDefault(s => s.OrderNumber == order_no && s.ProductCode == product_code);
            }
            catch (Exception x)
            {
                throw (x);
        ...
        private Boolean Save()
        {
            try
            {
                ...
                {
                    var x = new Order_RUTF_Details
                    {
                        OrderNumber = mOrderNumber,
                        ProductCode = mProductCode,
                        OpeningBalance = mOpeningBalance,
                        QuantityReceived = mQtyRecieved,
                        Consumption = mDispensed2MonthsReview,
                        LossesAdjustments = mLossesAndAdjustments,
                    ...
                    context.Order_RUTF_Details.Add(x);
                    context.SaveChanges();
                    return true;
                }
                else
        ...
        public Boolean Update()
        {
            try
            {
                ...
                if (x != null)
                {
                    ...
                    x.QuantityReceived = mQtyRecieved;
                    x.Consumption = mDispensed2MonthsReview;
                    x.LossesAdjustments = mLossesAndAdjustments;
