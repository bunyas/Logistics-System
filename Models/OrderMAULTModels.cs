...
namespace mascis.Models
{
    public class OrderMAULTModels
    {
        ...
        public Order_MAULT GetRecordByKey(string order_no, int? product_code)
        {
            try
            {
                return context.Order_MAULT.FirstOrDefault(s => s.OrderNumber == order_no && s.ProductCode == product_code);
            }
            catch (Exception x)
            {
                throw (x);
