...
{
    public class CHSS_MAULController : Controller
    {
        string Module;
        ...
        {
            List<Chss_M_ART_Patient_Care_Register> mlist = new List<Chss_M_ART_Patient_Care_Register>();
            if (string.IsNullOrEmpty(startMonth) & string.IsNullOrEmpty(endMonth)) { }
            else if (!string.IsNullOrEmpty(startMonth) & !string.IsNullOrEmpty(endMonth) & facilityid > 0)
            {
                DateTime sDate = DateTime.ParseExact(startMonth, "dd/MM/yyyy", null);
                DateTime eDate = DateTime.ParseExact(endMonth, "dd/MM/yyyy", null);
                mascisEntities db = new mascisEntities();
                var x = db.Chss_M_ART_Patient_Care_Register.ToList();
                var visits = db.Chss_Facility_Visits.ToList();
                if (startMonth == endMonth)
                {
                    var selectedVisits = visits.Where(e => e.date_of_visit.Date == sDate.Date & e.facility_code == facilityid).ToList();
                    if (selectedVisits.Count > 0)
                    {
                        foreach (var n in selectedVisits)
                        {
                            var m = x.Where(e => e.date_of_visit.Date == n.date_of_visit.Date && e.facility_code == n.facility_code).ToList();
                            if (m.Count > 0)
                            {
                                foreach (var a in m)
                                {
        ...
        {
            List<Chss_M_ART_Patient_Care_DispensingLog> mlist = new List<Chss_M_ART_Patient_Care_DispensingLog>();
            if (string.IsNullOrEmpty(startMonth) & string.IsNullOrEmpty(endMonth)) { }
            else if (!string.IsNullOrEmpty(startMonth) & !string.IsNullOrEmpty(endMonth) & facilityid > 0)
            {
                DateTime sDate = DateTime.ParseExact(startMonth, "dd/MM/yyyy", null);
                ...
                mascisEntities db = new mascisEntities();
                var visits = db.Chss_Facility_Visits.ToList();
                var z = db.Chss_M_ART_Patient_Care_DispensingLog.ToList();
                if (startMonth == endMonth)
                {
                    var selectedVisits = visits.Where(e => e.date_of_visit.Date == sDate.Date & e.facility_code == facilityid).ToList();
                    if (selectedVisits.Count > 0)
        ...
        {
            List<Chss_M_Stock_Management> StockList = new List<Chss_M_Stock_Management>();
            if (string.IsNullOrEmpty(startMonth) & string.IsNullOrEmpty(endMonth)) { }
            else if (!string.IsNullOrEmpty(startMonth) & !string.IsNullOrEmpty(endMonth) & facilityid > 0)
            {
                DateTime sDate = DateTime.ParseExact(startMonth, "dd/MM/yyyy", null);
